...
if ($proceed) {
    ...
    if ($settings['enable_payment_module']=='y' && check_allow('payments_budget_edit')) $optionlist[]='<A HREF="payments_budget_main.php" class="option">'.oicon('bank').lang('budgets').'</A>';
    ...
    $optionlist=array();
    if (check_allow('public_content_edit')) $optionlist[]='<A HREF="options_menu.php?area=public" class="option">'.oicon('list-alt').lang('menu_public_menu_and_pages').'</A>';
    if (check_allow('settings_view')) $optionlist[]='<A HREF="options_menu.php?area=admin" class="option">'.oicon('list-alt').lang('menu_admin_menu_and_pages').'</A>';
...
//  if (check_allow('default_text_edit')) $optionlist[]='<A HREF="lang_item_main.php?item=default_text" class="option">'.lang('default_texts').'</A>';
    ...
    $optionlist=array();
    if (check_allow('pform_config_field_configure')) $optionlist[]='<A HREF="options_participant_profile.php" class="option">'.oicon('wpforms').lang('participant_profile_fields').'</A>';
    if (check_allow('pform_templates_edit')) $optionlist[]='<A HREF="options_profile_template.php" class="option">'.oicon('newspaper-o').lang('participant_profile_form_template_legacy').'</A>';
    if (check_allow('pform_anonymization_fields_edit')) $optionlist[]='<A HREF="options_ordered_lists.php?list=anonymize_profile_list" class="option">'.oicon('user-secret').lang('fields_to_anonymize_in_anonymization_bulk_action').'</A>';
    ...
    if (check_allow('pform_lang_field_edit')) {
        ...
        foreach($formfields as $f) {
            if (preg_match("/(select_lang|radioline_lang|checkboxlist_lang)/",$f['type'])) {
                ...
                if ($field_label!=='') $to.=$field_label;
                else $to.=$f['mysql_column_name'];
                $to.='</A>';
                $optionlist[]=$to;
            }
        }
    ...
    if (check_allow('pform_results_lists_edit')) {
        ...
        if ($settings['enable_email_module']=='y') $optionlist[]='<A HREF="options_ordered_lists.php?list=email_participant_guesses_list" class="option">'.oicon('question-circle-o').lang('email_module_participant_guesses_list').'</A>';
        options__show_main_section(lang('result_lists'),$optionlist);
    }

    $optionlist=array();
    if (check_allow('import_data')) $optionlist[]='<A HREF="import_prepare.php" class="option">'.oicon('ambulance').lang('prepare_data_import').'</A>';
