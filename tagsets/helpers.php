...
function ortime__format($unixtime,$options='',$language='') {
    ...
    if (!isset($op['hide_date']) && !isset($op['hide_time'])) {
        if (lang__is_rtl($language)) $f=$tformat." ".$dformat;
        else $f=$dformat." ".$tformat;
    } else {
...
function ortime__date_parts_to_ymd($y,$m,$d,$mode='ymd') {
    if ($mode!=='ymd' && $mode!=='ym' && $mode!=='y') $mode='ymd';
    $yi=(int)$y;
    $mi=($mode==='y') ? 1 : (int)$m;
    $di=($mode==='ymd') ? (int)$d : 1;
...
function ortime__format_ymd_localized($ymd,$language='',$mode='ymd') {
    global $lang;
    $ymd=(string)$ymd;
    if (!preg_match('/^(\d{4})-(\d{2})-(\d{2})$/',$ymd,$dm)) return '';
    if ($mode==='y') return $dm[1];
    if ($mode==='ym') {
        if (!$language) {
            if (isset($lang['lang']) && $lang['lang']) $language=$lang['lang'];
        }
        ...
        if (!$format || $format==='format_datetime_date_no_day') $format='%m/%Y';
