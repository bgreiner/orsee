        ...
        if ($p['sess_id']!='0' || true) { //$last_reg_time > $now) {
            ...
            */
            ...
            if ($p['pstatus_id']>0) {
                ...
                if ($pstatuses[$p['pstatus_id']]['noshow']) echo 'var(--color-shownup-no)'; else echo 'var(--color-shownup-yes)';
                echo ';">';
            }
...
function participant__profile_field_is_applicable($field,$subpool_id,$scope_context='') {
    if (is_array($scope_context)) {
        ...
        if (!$scope_matches) return false;
    } elseif ($scope_context!=='' && !in_array($scope_context,$field['scope_contexts'],true)) return false;
    if (count($field['restrict_to_subpools'])>0 && !in_array((int)$subpool_id,$field['restrict_to_subpools'],true)) return false;
    return true;
}
...
function participant__profile_block_is_applicable($block,$subpool_id,$scope_context='') {
    ...
    if (!array_key_exists('scope_contexts',$block) || !is_array($block['scope_contexts'])) return false;
...
function participant__profile_fields_resolve_variant($formfields,$subpool_id,$scope_context='') {
    if (!is_array($formfields)) return array();
    if (!is_string($scope_context)) return $formfields;
    ...
    foreach ($formfields as $field) {
        ...
        foreach ($field['variants'] as $variant) {
            ...
            if (!isset($variant['scope_contexts']) || !is_array($variant['scope_contexts']) || !in_array($scope_context,$variant['scope_contexts'],true)) continue;
            if (!isset($variant['subpools']) || !is_array($variant['subpools']) || !in_array($subpool_id,$variant['subpools'],true)) continue;
...
function participantform__check_unique($edit,$formtype,$participant_id=0) {
    ...
    if ($use_layout_gate) {
        ...
        foreach ($layout['blocks'] as $block) {
            ...
            if (trim((string)$block['type'])!=='field' || !isset($block['field'])) continue;
            $field_name=trim((string)$block['field']);
            if ($field_name!=='') $active_layout_fields[$field_name]=true;
        }
    }
    ...
    foreach ($formfields as $f) {
        ...
        if ($check_this_field) {
            ...
            if ($f['mysql_column_name']==='email') {
                ...
                if (!in_array($email_input_mode,array('full_email','local_part'),true)) $email_input_mode='full_email';
                if ($email_input_mode==='local_part' && $email_fixed_domain!=='') {
                    ...
                    if ($email_value!=='') {
                        ...
                        if ($at_position!==false) $email_value=substr($email_value,0,$at_position);
            ...
            if ($f['type']=='boolean') {
                if (isset($edit[$f['mysql_column_name']]) && $edit[$f['mysql_column_name']]=='y') $edit[$f['mysql_column_name']]='y';
                else $edit[$f['mysql_column_name']]='n';
            }
        }
    }
...
function participant__profile_field_editor_render_controls($profile_specs, $field, $variant_mode=false, $variant_overrides=array(), $changed_keys=array()) {
    ...
    if (!is_array($variant_overrides)) $variant_overrides=array();
    $render_values=$field;
    ...
    foreach ($profile_specs['sections'] as $section_spec) {
        ...
        foreach ($profile_specs['fields'] as $key=>$field_spec) {
            if ((string)$field_spec['section_id']!==$section_id) continue;
            if (isset($display_fields) && is_array($display_fields) && !in_array($key,$display_fields,true)) continue;
        ...
        foreach ($profile_specs['fields'] as $key=>$field_spec) {
            ...
            if ($is_changed && !$variant_mode) $classes[]='orsee-track-changed-left';
            if (isset($field_spec['visibility_classes']) && is_array($field_spec['visibility_classes']) && count($field_spec['visibility_classes'])>0) {
                ...
                foreach ($visibility_classes as $vclass) {
                    ...
                    if ($vclass!=='') $classes[]=$vclass;
                }
            }

            $label=(string)$field_spec['label'];
            $row_html='<div class="'.implode(' ',$classes).'"';
            if ($tooltip!=='') $row_html.=' title="'.htmlspecialchars($tooltip,ENT_QUOTES).'"';
            $row_html.='>';
            ...
            if ($kind==='yesno') {
                ...
                if ($effective_type==='select_lang' && isset($render_values['order_select_lang_values']) && $render_values['order_select_lang_values']==='fixed_order') $lang_order='fixed_order';
                if (($effective_type==='radioline_lang' || $effective_type==='checkboxlist_lang') && isset($render_values['order_radio_lang_values']) && $render_values['order_radio_lang_values']==='fixed_order') $lang_order='fixed_order';
                ...
                if (in_array($key,$editable_fields,true)) {
                    ...
                    if ($size<1) $size=25;
                    $text_attrs='';
                    if ($force_ltr) $text_attrs.=' dir="ltr"';
                    if (isset($field_spec['force_ltr_if_type']) && is_array($field_spec['force_ltr_if_type'])) $text_attrs.=' data-force-ltr-if-type="'.implode(',',$field_spec['force_ltr_if_type']).'"';
                    ...
                    if ($default_value==='y') $row_html.=lang('y');
                    elseif ($default_value==='n') $row_html.=lang('n');
                    else $row_html.=lang('none');
                } elseif ($effective_type==='select_lang' || $effective_type==='radioline_lang') {
                    if ($default_value!=='' && isset($lang_options[$default_value])) $row_html.=htmlspecialchars((string)$lang_options[$default_value],ENT_QUOTES);
                    elseif ($default_value==='') $row_html.=lang('none');
                    else $row_html.=htmlspecialchars($default_value,ENT_QUOTES);
                ...
                if (isset($render_values[$key]) && is_array($render_values[$key])) {
                    foreach ($render_values[$key] as $pool_id) {
                        $pool_id=(int)$pool_id;
                        if ($pool_id>0 && !in_array($pool_id,$selected_subpools,true)) $selected_subpools[]=$pool_id;
                    }
                }
                $all_subpools=subpools__get_subpools();
                $subpool_options=array();
                foreach ($all_subpools as $pool) $subpool_options[(string)$pool['subpool_id']]=$pool['subpool_name'];
                ...
                if (in_array($key,$editable_fields,true)) {
                    ...
                    if (count($selected_subpools)===0) $selected_subpools=array_keys($subpool_options);
                    $selected_subpool_labels=array();
                    foreach ($selected_subpools as $selected_subpool_id) {
                        if (isset($subpool_options[$selected_subpool_id])) $selected_subpool_labels[]=$subpool_options[$selected_subpool_id];
                    }
                ...
                foreach ($languages as $language) {
                    ...
                    if (is_array($localized_value)) {
                        if (isset($localized_value[$language])) $lang_value=(string)$localized_value[$language];
                        ...
                        if ($legacy_symbol!=='') $lang_value=load_language_symbol($legacy_symbol,$language);
                    }
                ...
                if ($custom_html==='') $custom_html=(string)$field_spec['control']['html'];
                $resolve_path=function($source, $path) {
                    $parts=explode('.',trim((string)$path));
                    $cursor=$source;
                    foreach ($parts as $part) {
                        if ($part==='') continue;
                        if (!is_array($cursor) || !array_key_exists($part,$cursor)) return '';
                        $cursor=$cursor[$part];
                    }
                ...
                if (!in_array($date_mode,array('ymd','ym','y'),true)) $date_mode='ymd';
                if (in_array($key,$editable_fields,true)) {
                    ...
                    if ($date_value!=='') $row_html.=ortime__format_ymd_localized($date_value,'',$date_mode);
                }
            } elseif ($kind==='hidden') {
                if (in_array($key,$editable_fields,true)) {
            ...
            if ($variant_mode) {
                ...
                if (count($visibility_classes)>0) {
                    ...
                    foreach ($visibility_classes as $vclass) {
                        $vclass=trim((string)$vclass);
                        if ($vclass!=='') $variant_row_classes[]=$vclass;
                    }
                }
                $override_cell_class='orsee-form-row-col';
                if ($is_changed) $override_cell_class.=' orsee-track-changed-left';
    ...
    foreach ($profile_field_specs['fields'] as $k=>$field_spec) {
        ...
        if ($k==='option_values') {
            if (in_array('option_values',$editable_fields,true) && isset($input['option_values']) && is_array($input['option_values'])) {
                ...
                if (count($legacy_option_values)>0) $prop_array[$k]=array_combine($legacy_option_values,$legacy_option_symbols);
                else $prop_array[$k]=array();
            } else {
                $prop_array[$k]=$field[$k];
            }
            continue;
        ...
        if ($k==='email_mode') {
            if ($field_type==='email' && in_array('email_mode',$editable_fields,true)) {
                ...
                $email_fixed_domain=ltrim($email_fixed_domain,'@');
                $email_fixed_domain=str_replace(' ','',$email_fixed_domain);
                if ($email_input_mode==='local_part' && $email_fixed_domain==='') $email_input_mode='full_email';
            ...
            if (in_array($k,$editable_fields,true)) {
                if ($field_type==='textarea') {
                    ...
                    if ($field_type==='boolean') {
                        if (!in_array($default_value,array('','y','n'),true)) $default_value='';
                    } elseif ($field_type==='select_lang' || $field_type==='radioline_lang') {
                        $order='alphabetically';
                        if ($field_type==='select_lang' && isset($field['order_select_lang_values']) && $field['order_select_lang_values']==='fixed_order') $order='fixed_order';
                        if ($field_type==='radioline_lang' && isset($field['order_radio_lang_values']) && $field['order_radio_lang_values']==='fixed_order') $order='fixed_order';
                        $allowed_defaults=lang__load_lang_cat((string)$field['mysql_column_name'],lang('lang'),$order);
                        if ($default_value!=='' && !isset($allowed_defaults[$default_value])) $default_value='';
                        ...
                        if (isset($input[$k]) && is_array($input[$k])) {
                            foreach ($input[$k] as $selected_default) {
                                ...
                                if ($selected_default!=='' && isset($allowed_defaults[$selected_default])) $selected_defaults[]=$selected_default;
                            }
                        }
        ...
        if ($field_type==='date' && $k==='date_default_mode') {
            if (in_array($k,$editable_fields,true) && isset($input[$k])) {
                ...
                if (!in_array($date_default_mode,array('none','fixed','today'),true)) $date_default_mode='none';
        ...
        if (in_array($k,$editable_fields,true) && isset($input[$k])) {
            if (is_array($input[$k])) {
                foreach ($input[$k] as $tk=>$tv) {
                    if ($tv) $input[$k][$tk]=trim($tv);
                    else unset($input[$k][$tk]);
                }
...
function participant__profile_field_properties_normalize($properties, $profile_field_specs) {
    ...
    if (!array_key_exists('scope_contexts',$properties) && array_key_exists('admin_only',$properties)) {
        ...
        if ($legacy_admin_only==='y') $normalized['scope_contexts']=array('profile_form_admin_part');
        else $normalized['scope_contexts']=$public_scope_contexts;
    }

    // keep only known scope keys, unique, and in definition order
    if (isset($normalized['scope_contexts'])) {
        if (!is_array($normalized['scope_contexts'])) $scope_contexts_selected=array();
        else $scope_contexts_selected=array_map('trim',$normalized['scope_contexts']);
    ...
    if (!array_key_exists('restrict_to_subpools',$properties) && array_key_exists('subpools',$properties)) {
        ...
        if ($legacy_subpools==='' || $legacy_subpools==='all') $normalized['restrict_to_subpools']=array();
        else $normalized['restrict_to_subpools']=explode(',',$legacy_subpools);
    }
    if (isset($normalized['restrict_to_subpools'])) {
        if (!is_array($normalized['restrict_to_subpools'])) $normalized['restrict_to_subpools']=array();
    ...
    if (isset($normalized['email_mode'])) {
        ...
        if (!in_array($email_mode,array('full_email','local_part'),true)) $email_mode='full_email';
        ...
        if ($email_mode==='local_part' && $email_domain==='') $email_mode='full_email';
    ...
    if (isset($normalized['type']) && $normalized['type']==='date') {
        if ((!isset($normalized['date_default_mode']) || !$normalized['date_default_mode']) && isset($normalized['default_value']) && preg_match('/^\d{4}-\d{2}-\d{2}$/',(string)$normalized['default_value'])) {
            ...
            if (!isset($normalized['date_default_value']) || !$normalized['date_default_value']) $normalized['date_default_value']=$normalized['default_value'];
        }
...
function participant__field_localized_text($field,$json_property_name,$legacy_symbol_property_name,$language='') {
    ...
    if (count($text_lang)>0) {
        ...
        foreach ($text_lang as $txt) {
            if (trim((string)$txt)!=='') return trim((string)$txt);
        }
        return '';
    }
    if (!isset($field[$legacy_symbol_property_name]) || trim((string)$field[$legacy_symbol_property_name])==='') return '';
...
function participantform__load($layout_state='current') {
    ...
    if (!in_array($layout_state,array('current','draft'),true)) $layout_state='current';
    ...
    while ($line=pdo_fetch_assoc($result)) {
        ...
        if ($layout_state==='current' && $has_draft_baseline && !$has_current_baseline) continue;
        ...
        if ($layout_state==='draft') {
            ...
            if ($field_type==='') $field_type=$profile_field_specs['fields']['type']['default'];
        }
        if ($field_enabled!=='y') continue;
...
function participant__save_profile_layout($context='profile_form_public',$variant='draft',$layout=array()) {
    ...
    if (!isset($layout['blocks']) || !is_array($layout['blocks'])) $layout['blocks']=array();
    $item_name=participant__profile_layout_item_name($context,$variant);
...
function participant__profile_render_mode($force_mode='') {
    global $settings;
    if ($force_mode==='structured_layout' || $force_mode==='legacy_template') return $force_mode;
    if (!isset($settings['participant_profile_render_mode'])) return 'legacy_template';
    if ($settings['participant_profile_render_mode']!=='structured_layout') return 'legacy_template';
    return 'structured_layout';
}

function participant__profile_template_variant($template='current_template') {
    if ($template==='current_draft' || $template==='draft') return 'draft';
    return 'current';
}

function participant__render_profile_layout($layout,$fields,$field_policies=array(),$errors=array(),$changed_block_keys=array()) {
    global $settings;
    if (!is_array($layout) || !isset($layout['blocks']) || !is_array($layout['blocks'])) return '';
    ...
    foreach ($layout['blocks'] as $block) {
        ...
        if ($type==='section') {
            $section_block_key='';
            if (isset($block['block_id']) && trim((string)$block['block_id'])!=='') $section_block_key='section__'.trim((string)$block['block_id']);
            $section_changed=($section_block_key!=='' && isset($changed_block_keys[$section_block_key]));
            if ($section_seen && $section_open) $out.='</div>';
            $section_text='';
            ...
            if ($section_text!=='') {
                ...
                if ($section_changed) $section_title_class.=' orsee-track-changed-left';
            ...
            if ($content_emitted) $section_class.=' mt-2';
        ...
        if ($type==='text') {
            $text_block_key='';
            if (isset($block['block_id']) && trim((string)$block['block_id'])!=='') $text_block_key='text__'.trim((string)$block['block_id']);
            ...
            if ($text_changed) $text_field_class.=' orsee-track-changed-left';
        ...
        if ($field_name==='' || !isset($fields[$field_name])) continue;
        $label_key='';
        $text_before='';
        $text_after='';
        $help_key='';
        if (!isset($field_policies[$field_name]) || !is_array($field_policies[$field_name])) continue;
        ...
        if ($has_error) $field_class.=' orsee-field-error';
        $field_block_key='field__'.$field_name;
        if (isset($changed_block_keys[$field_block_key])) $field_class.=' orsee-track-changed-left';
        $out.='<div class="'.$field_class.'">';
        if ($label_key!=='') $out.='<label class="label">'.$label_key.'</label>';
        if ($help_key!=='') $out.='<p class="help">'.$help_key.'</p>';
        $out.='<div class="control">';
        if ($text_before!=='') $out.='<span>'.$text_before.' </span>';
        $out.=$fields[$field_name];
        if ($text_after!=='') $out.='<span> '.$text_after.'</span>';
        $out.='</div>';
        $out.='</div>';
        $content_emitted=true;
    }
...
function form__replace_funcs_in_field($f) {
    global $lang, $settings;
    foreach ($f as $o=>$v) {
        if (is_string($f[$o]) && substr($f[$o],0,5)=='func:') eval('$f[$o]='.substr($f[$o],5).';');
    }
    return $f;
}
...
function form__render_field($f) {
    ...
    switch($f['type']) {
        ...
        case 'email': $out=form__render_email($f); break;
        ...
        case 'checkboxlist_lang': $out=form__render_checkboxlist_lang($f); break;
        case 'boolean':
        ...
        case 'date':
            ...
            $voluntary=true;
            if (isset($f['compulsory']) && $f['compulsory']==='y') $voluntary=false;
        ...
        case 'phone':
            ...
            if ($phone_display_mode==='international') $phone_national_mode='false';
            if (!preg_match('/^[a-z]{2}$/',$default_country)) $default_country='us';
...
function form__render_select_list($f,$formfieldvarname='',$compact=false) {
    ...
    if ($compact) $select_class.=' select-compact';
...
function form__render_checkboxlist_lang($f) {
    ...
    $order='alphabetically';
    if (isset($f['order_radio_lang_values']) && $f['order_radio_lang_values']==='fixed_order') $order='fixed_order';
    ...
    foreach ($items as $val=>$text) {
        ...
        if (in_array((string)$val,$selected_values,true) || in_array((int)$val,$selected_values,true)) $out.=' checked';
        $out.='> '.$text.'</label>&nbsp;&nbsp;&nbsp;';
    }
    return $out;
}
...
function participant__subpool_subscription_exptypes($subpool_id,$enabled_only=true) {
    ...
    if (!isset($subpool['subpool_id']) || !$subpool['subpool_id']) return array();
    $subpool_exptypes=db_string_to_id_array($subpool['experiment_types']);
    if (!$enabled_only) return $subpool_exptypes;
    $enabled_exptypes=load_external_experiment_types();
    $filtered=array();
    foreach ($subpool_exptypes as $exptype_id) {
        if (isset($enabled_exptypes[$exptype_id])) $filtered[]=$exptype_id;
    }
    return $filtered;
}
...
function participant__select_numbers($ptablevarname,$formfieldvarname,$prevalue,$begin,$end,$fillzeros=2,$steps=1,$reverse=false,$incnone=false,$existing=false,$where='',$show_count=false,$compact=false) {
    $out='';
    if ($compact) $out.='<span class="select is-primary select-compact">';
    else $out.='<span class="select is-primary">';
    ...
    if (!$existing) {
        ...
        if (!is_numeric($steps) || (float)$steps<=0) $steps=1;
        $steps=(float)$steps;
        while (($reverse==false && $i<=$ub) || ($reverse==true && $i>=$lb)) {
            $ival=(float)$i;
            $out.='<option value="'.$ival.'"'; if ((string)$ival === (string)$prevalue) $out.=' SELECTED'; $out.='>';
            $out.=helpers__pad_number($i,$fillzeros); $out.='</option>';
            if ($reverse) $i=$ival-$steps; else $i=$ival+$steps;
            $i=round((float)$i,6);
        }
    } else {
        ...
        foreach ($listitems as $line) {
            $out.='<option value="'.$line['tf_value'].'"'; if ((string)$line['tf_value'] === (string)$prevalue) $out.=' SELECTED'; $out.='>';
...
function participant__show_form($edit,$button_title="",$errors=array(),$admin=false,$extra="") {
    ...
    echo '<table cellspacing="0" cellpadding="10em" border="0">
            <TR><TD>';
    if ($admin) participant__show_inner_form($edit,$errors,'profile_form_public_admin_edit');
    else participant__show_inner_form($edit,$errors,'profile_form_public_create');
    echo '</TD></TR>
            <TR><TD>';
    echo $extra;
    echo '</TD></TR>';
...
function participant__show_inner_form($edit,$errors=array(),$scope_context='',$template='current_template',$force_mode='',$show_changes=false) {
    ...
    if ($is_admin_ui) { $out['is_admin']=true; $out['is_not_admin']=false; }
    ...
    if ($is_admin_ui) $nonunique=participantform__get_nonunique($edit,$edit['participant_id'],$scope_context);
            ...
            if (isset($edit[$f['mysql_column_name']])) $f['value']=$edit[$f['mysql_column_name']];
                ...
                if ($f['type']==='date') {
                    ...
                    if ($default_mode==='today') {
                        ...
                        if ($default_date==='' && isset($f['default_value'])) $default_date=trim((string)$f['default_value']);
                        if (preg_match('/^\d{4}-\d{2}-\d{2}$/',$default_date)) $f['value']=$default_date;
                        else $f['value']='';
                    } else {
                        $f['value']='';
                    }
    ...
    if(in_array('language',$errors)) $out['error_language']=' style="background: var(--color-missing-field);"';
    ...
    if (!$hide_subscriptions) {
        ...
        if(in_array('subscriptions',$errors)) $out['error_subscriptions']=' style="background: var(--color-missing-field);"';
    ...
    if (participant__profile_render_mode($force_mode)==='structured_layout') {
        ...
        foreach ($layout['blocks'] as $block) {
            ...
            if ($block_type==='text' || $block_type==='section') {
                if (!participant__profile_block_is_applicable($block,$subpool['subpool_id'],$scope_context)) continue;
            }
            $filtered_blocks[]=$block;
        }
        $layout['blocks']=$filtered_blocks;
        $errors_map=array();
        foreach ($errors as $err_name) $errors_map[$err_name]=true;
...
function participant__get_inner_admin_form($edit,$errors,$template='current_template',$force_mode='',$show_changes=false) {
    ...
    $layout_state=(participant__profile_render_mode($force_mode)==='structured_layout' ? participant__profile_template_variant($template) : 'current');
    $formfields=participantform__load($layout_state); $tout=array();
    ...
    if (participant__profile_render_mode($force_mode)==='structured_layout') {
        ...
        foreach ($layout['blocks'] as $block) {
            ...
            $block_type=trim((string)$block['type']);
            if ($block_type==='text' || $block_type==='section') {
                if (!participant__profile_block_is_applicable($block,$subpool['subpool_id'],'profile_form_admin_part')) continue;
            }
            $filtered_blocks[]=$block;
        }
        $layout['blocks']=$filtered_blocks;
...
function participant__check_login($email,$password) {
    ...
    if ($continue) {
        ...
        if (isset($locked) && $locked) message(lang('error_locked_out'),'error');
        return false;
    }
}
        ...
        if ($col=='pform_fields') {
            foreach ($formfields as $f) {
                if (!isset($cols[$f['mysql_column_name']]) && !isset($ptable_columns[$f['mysql_column_name']])) {
                    ...
                    if (preg_match("/(radioline|select_list|select_lang|radioline_lang|select_numbers|boolean)/",$f['type'])) $session_editable=true;
                    $display_text=trim((string)participant__field_localized_text($f,'name_lang','name_lang'));
                    if ($display_text==='') $display_text=lang('mysql_column_name');
    ...
    foreach ($columns as $k=>$arr) {
        ...
        switch($k) {
            ...
            case 'status_id':
                ...
                break;
            case 'edit_link':
                if (check_allow('participants_edit')) $out.='<div class="orsee-table-cell orsee-table-action"></div>';
                break;
            default:
...
function participant__get_result_table_grid_template($columns,$with_leading_spacer=false) {
    $tracks=array();
    if ($with_leading_spacer) $tracks[]='minmax(0.8rem, 1.2rem)';
    foreach ($columns as $k=>$arr) {
        switch($k) {
            case 'checkbox':
...
function participant__get_result_table_row($columns,$p,$select_wrapper_class='select is-primary',$editable_session_columns=array()) {
    ...
    foreach ($columns as $k=>$arr) {
        $cell_label='';
        if (isset($arr['display_table_head'])) $cell_label=$arr['display_table_head'];
        elseif (isset($arr['display_text'])) $cell_label=$arr['display_text'];
        if (isset($pform_columns[$k]['column_name'])) $cell_label=$pform_columns[$k]['column_name'];
        if ($k=='checkbox') $cell_label=lang('select');
        elseif ($k=='status_id') $cell_label=lang('participant_status_abbr');
        elseif ($k=='edit_link') $cell_label=lang('action');
        elseif ($k=='order_number') $cell_label='';
        ...
        if (in_array($expadmindata['admin_type'],$hide_for_admin_types)) {
            ...
            switch($k) {
                ...
                case 'checkbox':
                    ...
                    if (isset($_REQUEST['sel'][$p['participant_id']]) && $_REQUEST['sel'][$p['participant_id']]=='y') $content.=' CHECKED';
                    elseif (isset($_SESSION['sel'][$p['participant_id']]) && $_SESSION['sel'][$p['participant_id']]=='y') $content.=' CHECKED';
                    $content.='>';
                ...
                case 'number_noshowup':
                    ...
                    break;
                case 'order_number':
                    if (isset($p[$k])) $out.=$cell($p[$k],'padding-inline-end: 0.35rem;',$cell_label);
                    else $out.=$cell('','padding-inline-end: 0.35rem;',$cell_label);
                    break;
                case 'invited':
                ...
                case 'rules_signed':
                    if ($settings['enable_rules_signed_tracking']=='y')  {
                        if (isset($editable_session_columns['rules_signed']) && $editable_session_columns['rules_signed']) {
                            ...
                            if ($p['rules_signed']=='y') $content=lang('y'); else $content=lang('n');
                        }
                        $out.=$cell($content,'',$cell_label);
                    }
                    break;
                ...
                case 'status_id':
                    ...
                    if ($participant_statuses[$p['status_id']]['eligible_for_experiments']=='y') $ccolor='var(--color-participant-status-eligible)';
                    else $ccolor='var(--color-participant-status-noneligible)';
                ...
                case 'last_login_attempt':
                    if ($p[$k]) $out.=$cell(ortime__format($p[$k],'hide_second:false'),'white-space: nowrap;', $cell_label);
                    else  $out.=$cell('-', 'white-space: nowrap;', $cell_label);
                    break;
                case 'session_id':
                    $content='';
                    ...
                    } else $content.=session__build_name($thislist_sessions[$p['session_id']]);
                    $out.=$cell($content,'',$cell_label);
                    break;
                case 'payment_budget':
                    if($settings['enable_payment_module']=='y') {
                        ...
                        if (check_allow('payments_edit')) {
                            ...
                            if (isset($payment_budgets[$p['payment_budget']])) $out.=$cell($payment_budgets[$p['payment_budget']]['budget_name'],'',$cell_label); else $out.=$cell('-','',$cell_label);
                        }
                    }
                    break;
                case 'payment_type':
                    if($settings['enable_payment_module']=='y') {
                        ...
                        if (check_allow('payments_edit')) {
                            ...
                            if (isset($payment_types[$p['payment_type']])) $out.=$cell($payment_types[$p['payment_type']],'',$cell_label); else $out.=$cell('-','',$cell_label);
                        }
                    }
                    break;
                case 'payment_amount':
                    if($settings['enable_payment_module']=='y') {
                        if (check_allow('payments_edit')) {
                            ...
                            if ($p['payment_amt']!='') $content.=$p['payment_amt']; else $content.='0.00';
                            $content.='" size="7" maxlength="10" style="text-align:end;">';
                            $out.=$cell($content,'',$cell_label);
                        } elseif (check_allow('payments_view')) {
                            if ($p['payment_amt']!='') $out.=$cell($p['payment_amt'],'',$cell_label); else $out.=$cell('-','',$cell_label);
                        }
                    }
                    break;
                case 'pstatus_id':
                    ...
                    if (isset($pform_columns[$k])) {
                        ...
                        if ($allow_session_edit && preg_match("/(radioline|select_list|select_lang|radioline_lang|select_numbers|boolean)/",$field_type)) {
                            if ($field_type=='select_list' || $field_type=='radioline') {
                                ...
                                if ($p[$k]==='y') $content.=' checked';
                                $content.='></label>';
                            } elseif ($field_type=='select_lang' || $field_type=='radioline_lang') {
                                if ($pform_columns[$k]['include_none_option']=='y') $incnone=true; else $incnone=false;
                                ...
                                if ($pform_columns[$k]['include_none_option']=='y') $incnone=true; else $incnone=false;
                                if ($pform_columns[$k]['values_reverse']=='y') $reverse=true; else $reverse=false;
                                $content='<span class="'.$select_wrapper_class.'">'.
                            ...
                            if($pform_columns[$k]['link_as_email_in_lists']=='y') $content.='<A HREF="mailto:'.$p[$k].'">';
                            if($field_type==='boolean') {
                                if ($p[$k]==='y') $content.=lang('y'); else $content.=lang('n');
                            } elseif($field_type==='checkboxlist_lang') {
                                ...
                                if (isset($pform_columns[$k]['lang'][$p[$k]])) $content.=lang($pform_columns[$k]['lang'][$p[$k]]);
                                else $content.=$p[$k];
                                ...
                                if ($date_value) $content.=$date_value; else $content.=$p[$k];
                                $out.=$cell($content,'white-space: nowrap;',$cell_label);
                                $cell_rendered=true;
                                $content='';
                            } else $content.=$p[$k];
                            if($pform_columns[$k]['link_as_email_in_lists']=='y') $content.='</A>';
                        }
                        if (!$cell_rendered) $out.=$cell($content,'',$cell_label);
                    } else {
                        if (isset($p[$k])) $out.=$cell($p[$k],'',$cell_label);
                        else $out.=$cell('???','',$cell_label);
                    }
            }
        }
    }
                ...
                default:
                    if (isset($pform_columns[$k])) {
                        if ($pform_columns[$k]['type']==='boolean') {
                            ...
                            $date_mode=(isset($pform_columns[$k]['date_mode']) ? $pform_columns[$k]['date_mode'] : 'ymd');
                            $date_value=ortime__format_ymd_localized((string)$p[$k],'',$date_mode);
                            if ($date_value) $row[]=$date_value; else $row[]=$p[$k];
                        } else $row[]=$p[$k];
                    } else {
...
function participant__load_all_pform_fields($tlang='') {
    ...
    if (isset($preloaded_all_pform_fields[$tlang]) && is_array($preloaded_all_pform_fields[$tlang])) return $preloaded_all_pform_fields[$tlang];
        ...
        foreach ($formfields as $f) {
            ...
            if (!$f['column_name']) $f['column_name']=$f['mysql_column_name'];
