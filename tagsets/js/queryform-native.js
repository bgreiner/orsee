    ...
    for (var key in obj[i]) {
      if (typeof Ptypes[key] !== "undefined") {
        ...
        if (typeof tr !== "undefined") {
          for (var elemKey in obj[i][key]) {
            if (elemKey == "logical_op") {
              var lopSelect = tr.children[logop_index]
                ? tr.children[logop_index].querySelector("select")
                : null;
              if (lopSelect) lopSelect.value = obj[i][key][elemKey];
            } else {
              var targets = _qfAll(
                "[name='" + elemKey + "'], [data-elem-name='" + elemKey + "']",
                tr
              );
        ...
        if (typeof obj[i][key]["logical_op"] !== "undefined") {
          var trOpenSel = trOpen.children[logop_index]
            ? trOpen.children[logop_index].querySelector("select")
            : null;
...
function moveUp(tr) {
  if (tr.getAttribute("data-row-type") == "bracket_close") {
    var open = _qfFirst(
      "[data-group-id='" + tr.getAttribute("data-group-id") + "'][data-row-type='bracket_open']"
    );
    if (open && _qfRowPos(tr) < _qfRowPos(open) + 2) {
      ...
      if (prev && prev.getAttribute("data-group-id") === null) {
        ...
        var prevOpen = _qfFirst(
          "[data-group-id='" + prev.getAttribute("data-group-id") + "'][data-row-type='bracket_open']"
        );
    ...
    var closeRow = _qfFirst(
      "[data-group-id='" + tr.getAttribute("data-group-id") + "'][data-row-type='bracket_close']"
    );
    if (prevRow && closeRow) {
...
function moveDown(tr) {
  if (tr.getAttribute("data-row-type") == "bracket_open") {
    ...
    if (!next) {
      ...
      next.getAttribute("data-row-type") != "bracket_close" &&
      next.getAttribute("data-row-type") != "bracket_open"
    ) {
      _qfInsertAfter(tr, next);
    } else if (next.getAttribute("data-row-type") == "bracket_close") {
      //do nothing
    } else {
      var nextClose = _qfFirst(
        "[data-group-id='" + next.getAttribute("data-group-id") + "'][data-row-type='bracket_close']"
      );
  ...
  if (elementType == "randsubset_limitnumber") {
    ...
    _qfAll("*", tr).forEach(function (el) {
      if (typeof el.getAttribute("id") !== "undefined" && el.getAttribute("id") !== null) {
        el.setAttribute(
          "id",
          el.getAttribute("id").replace(Ptypes[elementType]["placeholder"], "query_item_" + queryCount)
        );
      }
      if (typeof el.getAttribute("class") !== "undefined" && el.getAttribute("class") !== null) {
        el.setAttribute(
          "class",
          el.getAttribute("class").replace(Ptypes[elementType]["placeholder"], "query_item_" + queryCount)
        );
      }
      ...
      if (typeof el.getAttribute("name") !== "undefined" && el.getAttribute("name") !== null) {
        el.setAttribute(
          "name",
          el.getAttribute("name").replace(Ptypes[elementType]["placeholder"] + "_", "")
        );
      }
    });
